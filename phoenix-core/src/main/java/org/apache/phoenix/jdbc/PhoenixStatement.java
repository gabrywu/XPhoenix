...
public class PhoenixStatement implements Statement, SQLCloseable {
    ...
    protected static class ExecutableNodeFactory extends ParseNodeFactory {
        ...
        public ExecutableDeleteStatement delete(NamedTableNode table, HintNode hint, ParseNode whereNode, List<OrderByNode> orderBy, LimitNode limit, int bindCount, Map<String, UDFParseNode> udfParseNodes) {
            // todo 2019-07-24 18:28:48 仔细研究在这里处理是否合理
            HintNode newHintNode = hint;
            if(whereNode!=null
                    && !ExternalConstants.EXTERNAL_QUERY_FIELD.equals(IndexUtil.getDataColumnName(whereNode.getChildren().get(0).toString()))){
                newHintNode = HintNode.create(hint==null?HintNode.EMPTY_HINT_NODE: hint, HintNode.Hint.NO_INDEX);
            }
            return new ExecutableDeleteStatement(table, newHintNode, whereNode, orderBy, limit, bindCount, udfParseNodes);
        }
