...
import org.apache.phoenix.external.iterate.ExternalIterators;
...
public class ScanPlan extends BaseQueryPlan {
    ...
    protected ResultIterator newIterator(ParallelScanGrouper scanGrouper, Scan scan, Map<ImmutableBytesPtr,ServerCache> caches) throws SQLException {
        ...
        if(ExternalUtil.isExternalIndex(table)){
            iterators = new ExternalIterators(this, perScanLimit, scanGrouper, scan, initFirstScanOnly, caches, dataPlan);
        }else if (isOffsetOnServer) {
