...
import org.apache.phoenix.external.index.LoggingExternalBatchableIndexTableFactory;
...
public class MutationState implements SQLCloseable {
    ...
    private ExternalIndexTableBatcherFactory externalIndexFactory;
    ...
    MutationState(long maxSize, long maxSizeBytes, PhoenixConnection connection,
        ...
        Configuration configuration = connection.getQueryServices().getConfiguration();
        externalIndexFactory = ExternalClassFactory.getInstance(
                configuration.get(ExternalConstants.EXTERNAL_INDEX_BATCH_FACTORY_CLASS_KEY
                        ,ExternalConstants.DEFAULT_EXTERNAL_INDEX_BATCH_FACTORY_CLASS),
                ExternalIndexTableBatcherFactory.class);
        externalIndexFactory.initialize(configuration);
    }
    ...
    private void generateMutations(final TableRef tableRef, final long mutationTimestamp, final long serverTimestamp,
        ...
        while (iterator.hasNext()) {
            ...
            }else if (state.getRowTimestampColInfo().getTimestamp() != null){
                timestampToUse = state.getRowTimestampColInfo().getTimestamp();
            }
    ...
    private void batchExternalIndex(final PTable index,final List<Mutation> mutationBatch) throws IOException, InterruptedException {
        IExternalBatch batch = externalIndexFactory.createExternalIndexTableBatch(index);
        batch.open();
        try {
    ...
    private void batch(final HTableInterface finalHTable,
                       final TableInfo finalTableInfo,
                       final List<Mutation> mutationBatch) throws IOException, InterruptedException {
        if(!finalTableInfo.isDataTable()){
            PTable indexTable = SchemaUtil.findIndex(finalTableInfo.getOrigTableRef().getTable(),finalHTable.getName());
            if(indexTable!=null && PTable.IndexType.EXTERNAL == indexTable.getIndexType()){
                batchExternalIndex(indexTable,mutationBatch);
            }else{
                finalHTable.batch(mutationBatch);
            }
        }else if (PTableType.INDEX == finalTableInfo.getOrigTableRef().getTable().getType()
                && PTable.IndexType.EXTERNAL == finalTableInfo.getOrigTableRef().getTable().getIndexType()){
            batchExternalIndex(finalTableInfo.getOrigTableRef().getTable(),mutationBatch);
                    ...
                    try {
                        ...
                        for (final List<Mutation> mutationBatch : mutationBatchList) {
                            if (shouldRetryIndexedMutation) {
                                ...
                                final TableInfo finalTableInfo = tableInfo;
                                PhoenixIndexFailurePolicy.doBatchWithRetries(new MutateCommand() {
                                    @Override
                                    public void doMutation() throws IOException {
                                        try {
                                            batch(finalHTable,finalTableInfo,mutationBatch);
                                        } catch (InterruptedException e) {
                                            Thread.currentThread().interrupt();
                                            throw new IOException(e);
                                        }
                                ...
                                batch(hTable,tableInfo,mutationBatch);
                            }
