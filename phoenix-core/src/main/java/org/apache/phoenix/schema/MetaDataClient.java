...
public class MetaDataClient {
    ...
    private static final String ADD_EXTERNAL_COLUMN_FORMAT = "alter table %s add "+
            SchemaUtil.getQuotedFullColumnName(QueryConstants.DEFAULT_COLUMN_FAMILY ,ExternalConstants.EXTERNAL_QUERY_FIELD )+" varchar";

    private static final String DROP_EXTERNAL_COLUMN_FORMAT = "alter table %s drop column "+
            SchemaUtil.getQuotedFullColumnName(QueryConstants.DEFAULT_COLUMN_FAMILY ,ExternalConstants.EXTERNAL_QUERY_FIELD );
    ...
    public MetaDataClient(PhoenixConnection connection) {
        ...
        this.externalMetaDataClient = ExternalClassFactory.getInstance(connection.getQueryServices().getConfiguration().get(
                ExternalConstants.EXTERNAL_METADATA_CLASS_KEY,
                ExternalConstants.DEFAULT_EXTERNAL_METADATA_CLASS), ExternalMetaDataClient.class);
        externalMetaDataClient.initialize(this);
    }
    ...
    private MutationState buildIndexAtTimeStamp(PTable index, NamedTableNode dataTableNode) throws SQLException {
        if( IndexType.EXTERNAL == index.getIndexType()){
            return externalMetaDataClient.buildIndexAtTimeStamp(index,dataTableNode);
        }
    ...
    private MutationState buildIndex(PTable index, TableRef dataTableRef) throws SQLException {
        if( IndexType.EXTERNAL == index.getIndexType()){
            return externalMetaDataClient.buildIndex(index,dataTableRef);
        }
    ...
    public MutationState createIndex(CreateIndexStatement statement, byte[][] splits) throws SQLException {
        ...
        Map<String,Object> commonFamilyProps = Maps.newHashMapWithExpectedSize(statement.getProps().size() + 1);
        populatePropertyMaps(statement.getProps(), tableProps, commonFamilyProps);
        tableProps.put(ExternalConstants.ATTRIBUTE_INDEX_TYPE,statement.getIndexType());
        ...
        while (true) {
            try {
                if(IndexType.EXTERNAL == statement.getIndexType()){
                    // 源表增加 EXTERNAL_QUERY 字段
                    String alterDataTableDdl = String.format(ADD_EXTERNAL_COLUMN_FORMAT,statement.getTable().getName().toString());
                    logger.debug("Add {} field for {} ,sql {}",
                            ExternalConstants.EXTERNAL_QUERY_FIELD ,statement.getTable().getName(),alterDataTableDdl);
                    connection.createStatement().execute(alterDataTableDdl);
                }
                ...
                if(IndexType.EXTERNAL == statement.getIndexType() && !externalMetaDataClient.isSupported(dataTable)){
                    logger.error("External index ["+externalMetaDataClient.getType()+"] not support transaction for "+dataTable.getName() + "!!!" );
                    logger.error("You index may be incorrect !!!");
                }
                ...
                StatementContext context = new StatementContext(phoenixStatement, resolver);
        ...
        if(IndexType.EXTERNAL == statement.getIndexType()){
            externalMetaDataClient.createIndex(table,tableProps);
        }
    ...
    private void dropExternalIndexSilent(PTable pTable,String schemaName,String parentTableName){
        if(PTableType.INDEX == pTable.getType()
                && IndexType.EXTERNAL == pTable.getIndexType()){
            try {
                externalMetaDataClient.dropIndex(pTable);
                if(parentTableName != null){
                    // 源表删除 EXTERNAL_QUERY 字段
                    String alterDataTableDdl = String.format(DROP_EXTERNAL_COLUMN_FORMAT,
                            SchemaUtil.getQualifiedTableName(schemaName,parentTableName));
                    logger.debug("Drop {} field for {} ,sql {}",
                            ExternalConstants.EXTERNAL_QUERY_FIELD ,SchemaUtil.getQualifiedTableName(schemaName,parentTableName),alterDataTableDdl);
                    connection.createStatement().execute(alterDataTableDdl);
                }
            } catch (SQLException e) {
                logger.warn(e.getMessage(),e);
            }
        }else if(PTableType.TABLE == pTable.getType()){
            for (PTable index:pTable.getIndexes()){
                dropExternalIndexSilent(index,null,null);
            }
        }
    }
            ...
            default:
                ...
                if (table != null) {
                    ...
                    if (!dropMetaData) {
                        ...
                        dropExternalIndexSilent(table,schemaName,parentTableName);
                        return state;
                    }else{
                        dropExternalIndexSilent(table,schemaName,parentTableName);
                    }
