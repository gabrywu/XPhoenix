...
create_index_node returns [CreateIndexStatement ret]
    :   CREATE (l=LOCAL | l=EXTERNAL)? INDEX (IF NOT ex=EXISTS)? i=index_name ON t=from_table_name
        (LPAREN ik=ik_constraint RPAREN)
        (INCLUDE (LPAREN icrefs=column_names RPAREN))?
        (async=ASYNC)?
        (p=fam_properties)?
        (SPLIT ON v=value_expression_list)?
        {ret = factory.createIndex(i, factory.namedTable(null,t), ik, icrefs, v, p, ex!=null, IndexType.fromToken(l==null?"":l.getText()), async != null, getBindCount(), new HashMap<String, UDFParseNode>(udfParseNodes)); }
    ;
