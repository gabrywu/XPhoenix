...
public class CreateTableIT extends ParallelStatsDisabledIT {
    ...
    @Test
    public void testCreateTableWithRowTimestamp() throws Exception {
        String schemaName = "TEST";
        String tableName = schemaName + generateUniqueName();
        Properties props = new Properties();

        String ddl =
                "CREATE TABLE " + tableName + "(addtime VARCHAR ,\n"
                        + "                dir ROW_TS ,\n"
        ...
        try (Connection conn = DriverManager.getConnection(getUrl(), props);) {
            conn.createStatement().execute("upsert into " + tableName+"(id,dir,addtime) values('123',123113,'test')");
            conn.commit();
        }
        try (Connection conn = DriverManager.getConnection(getUrl(), props);) {
            ResultSet rs = conn.createStatement().executeQuery("SELECT ID,DIR,ADDTIME FROM " + tableName);
            ResultSetMetaData rsm = rs.getMetaData();
            assertEquals(3,rsm.getColumnCount());
            for (int i = 1; i <= 3; i++) {
                System.out.println("name="+rsm.getColumnName(i)+",type="+rsm.getColumnType(i)+",typeName="+rsm.getColumnTypeName(i));
            }
            assertTrue(rs.next());
            assertEquals("123", rs.getString (1));
            assertEquals(123113L, rs.getLong(2));
            assertEquals("test", rs.getString(3));
            assertFalse(rs.next());
        }
        try(org.apache.hadoop.hbase.client.Connection connection = utility.getConnection()){
            TableName [] tableNames = connection.getAdmin().listTableNames();

            Table table = connection.getTable(TableName.valueOf("default",tableName) );
            Scan scan = new Scan();
            ...
            List<Cell> rsCells = res.listCells();

            assertTrue(rsCells.size()==3);
            for (Cell cell:rsCells){

                System.out.println("row "+ Bytes.toString(CellUtil.cloneRow(cell)) +",column: "+Bytes.toString(CellUtil.cloneQualifier(cell))
                        +",value: "+(Bytes.toString(CellUtil.cloneQualifier(cell)).equals("DIR") ? Bytes.toLong(CellUtil.cloneValue(cell)):Bytes.toString(CellUtil.cloneValue(cell)))+",timestamp: "+cell.getTimestamp());
            }
            assertEquals(123113L,  rsCells.get(0).getTimestamp());
