...
import static org.apache.phoenix.external.index.elasticsearch.base.TestUtil.*;
...
public abstract class BaseTest {
    ...
    public static void populateTestTable2(String fullTableName) throws SQLException{
        populateTestTable(fullTableName);
        Properties props = PropertiesUtil.deepCopy(TEST_PROPERTIES);
        try (Connection conn = DriverManager.getConnection(getUrl(), props)) {
            upsertRow( conn, fullTableName, 4, false);
            conn.createStatement().executeUpdate("upsert into "+fullTableName+
                    "(VARCHAR_PK,CHAR_PK,INT_PK,LONG_PK,DECIMAL_PK,DATE_PK,INT_COL2,char_col2)" +
                    "values('varchar4','char4',4,4,4,'2015-01-01 00:00:00',5,'charb4')");
            conn.createStatement().executeUpdate("upsert into "+fullTableName+
                    "(VARCHAR_PK,CHAR_PK,INT_PK,LONG_PK,DECIMAL_PK,DATE_PK,char_col2)" +
                    "values('varchar3','char3',3,3,3,'2015-01-01 00:00:00','charb1')");
            conn.commit();
        }
    }
